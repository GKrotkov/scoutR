...
linear_weight_fits <- lapply(matches, weighted_fit, w = w)
nonlinear_weight_fits <- lapply(matches, weighted_fit, w = 1 / bin_vars)
raw_fits <- lapply(matches, weighted_fit, w = 1)

linear_weight_loocvs <- sapply(linear_weight_fits, LOOCV)
nonlinear_weight_loocvs <- sapply(nonlinear_weight_fits, LOOCV)
raw_loocvs <- sapply(raw_fits, LOOCV)

viz <- data.frame(linear_loocv_diff = linear_weight_loocvs - raw_loocvs, 
                  nonlinear_loocv_diff = nonlinear_weight_loocvs - raw_loocvs,
                  year = as.factor(substr(names(raw_loocvs), 1, 4)))

ggplot(viz, aes(x = nonlinear_loocv_diff)) + 
    geom_histogram(fill = gos_blue, color = "black", 
                   bins = ceiling(sqrt(nrow(viz)))) + 
    geom_vline(aes(xintercept = mean(nonlinear_loocv_diff)), color = gos_red) + 
    annotate("label", x = mean(viz$nonlinear_loocv_diff), y = 15, 
             label = paste0("Mean: ", 
                            round(mean(viz$nonlinear_loocv_diff), 3))) +
    labs(title = "Weighting Improves OPR Predictions", 
         subtitle = "District Event Qualifications, 2009 - 2024", 
...
viz <- viz %>%
    group_by(year) %>%
    summarize(events = n(), 
              linear_mean_diff = mean(linear_loocv_diff), 
              linear_median_diff = median(linear_loocv_diff), 
              nonlinear_mean_diff = mean(nonlinear_loocv_diff), 
              nonlinear_median_diff = median(nonlinear_loocv_diff))

colnames(viz) <- c(
    "Year", "# Events", 
    "Mean Linear LOOCV Difference", "Median Linear LOOCV Difference", 
    "Mean Nonlinear LOOCV Difference", "Median Nonlinear LOOCV Difference"
)
...
```{r}
mean(nonlinear_weight_loocvs - raw_loocvs) / 
