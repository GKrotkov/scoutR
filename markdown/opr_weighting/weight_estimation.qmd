...
viz <- data.frame(bin = 1:length(bin_vars), 
                  bin_vars = bin_vars, bin_fitteds = bin_fitteds)

ggplot(viz, aes(x = bin, y = bin_vars)) + 
    geom_point() + 
    labs(title = "Residual Variance by Binned Tournament Progress", 
         x = "Bin Number", y = "Residual Variance") +
    scale_x_continuous(breaks = viz$bin) +
    gos_theme

ggplot(viz, aes(x = bin, y = bin_fitteds)) + 
    geom_point() + 
    labs(title = "Fitted Value Variance by Binned Tournament Progress", 
         x = "Bin Number", y = "Normalized Residual Variance") +
    scale_x_continuous(breaks = viz$bin) +
    gos_theme

bin_vars22 <- rep(0, length(unique(cuts)))
resids22 <- unlist(residuals)[startsWith(names(residuals), "2022")]
cuts22 <- cuts[startsWith(names(residuals), "2022")]
# repeat for 2022 specifically
for (i in 1:length(unique(cuts))){
    bin_vars22[i] <- var(resids22[cuts == levels(cuts)[i]], na.rm = TRUE)
}
```
...
ggplot(viz, aes(x = bin, y = bin_vars)) + 
    geom_point() + 
    geom_abline(intercept = coefficients(first_half_fit)[1], 
                slope = coefficients(first_half_fit)[2], 
                col = gos_blue) +
    geom_abline(intercept = coefficients(second_half_fit)[1], 
                slope = coefficients(second_half_fit)[2], 
                col = gos_red) +
    labs(title = "Two lines approximate the variance relationship", 
         x = "Bin Number", y = "Residual Variance") +
    scale_x_continuous(breaks = viz$bin) + 
    gos_theme
