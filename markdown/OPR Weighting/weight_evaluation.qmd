...
nonlinear_weighted_loocvs <- sapply(nonlinear_weight_fits, LOOCV)
raw_loocvs <- sapply(raw_fits, LOOCV)

viz <- data.frame(loocv_diff = weighted_loocvs - raw_loocvs, 
                  nonlinear_loocv_diff = nonlinear_weighted_loocvs - raw_loocvs,
...
Given the differences in means; I would recommend using the `1 / bin_vars` as the weighting rather than the linear approximation; I trust the LOOCV evaluation over the test MSE evaluation.

```{r}
mean(viz$nonlinear_loocv_diff)
mean(viz$loocv_diff)
```
